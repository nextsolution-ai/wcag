(function(){const defaultConfig={position:"right",theme:"light"};const config={...defaultConfig,...window.WCAGWidgetConfig||{}};function loadStyles(){const styleLink=document.createElement("link");styleLink.rel="stylesheet";styleLink.href="https://nextsolution-ai.github.io/wcag/dist/style.min.css";document.head.appendChild(styleLink);const profilesLink=document.createElement("link");profilesLink.rel="stylesheet";profilesLink.href="https://nextsolution-ai.github.io/wcag/dist/profiles.min.css";document.head.appendChild(profilesLink)}function createAccessibilityWidget(){const widgetHTML=`
    ...
    `;document.body.insertAdjacentHTML("beforeend",widgetHTML)}function initializeWidget(){if(!document.getElementById("accessibility-widget")){createAccessibilityWidget()}const btn=document.getElementById("accessibility-btn");const panel=document.getElementById("accessibility-panel");const closeBtn=document.getElementById("daccheac");const widget=document.getElementById("accessibility-widget");const languageSelect=document.getElementById("language-select");const positionSelect=document.getElementById("position-select");if(!btn||!panel||!closeBtn||!widget){console.error("WCAG Widget: Required elements not found");return}if(config.position==="left"){widget.style.right="";widget.style.left="32px";positionSelect.value="left"}else{widget.style.left="";widget.style.right="32px";positionSelect.value="right"}btn.addEventListener("click",()=>{const isOpen=panel.classList.toggle("open");panel.setAttribute("aria-hidden",!isOpen)});closeBtn.addEventListener("click",()=>{panel.classList.remove("open");panel.setAttribute("aria-hidden","true")});document.querySelectorAll(".accordion-toggle").forEach(btn=>{btn.addEventListener("click",function(){const content=document.getElementById(this.getAttribute("aria-controls"));const expanded=this.getAttribute("aria-expanded")==="true";this.setAttribute("aria-expanded",!expanded);content.hidden=expanded})});positionSelect.addEventListener("change",function(){if(this.value==="left"){widget.style.right="";widget.style.left="32px"}else{widget.style.left="";widget.style.right="32px"}});document.querySelectorAll(".profile-btn").forEach(btn=>{btn.addEventListener("click",function(){const profile=this.getAttribute("id");switch(profile){case"daccpmi":document.body.style.animation="none";break;case"daccpbl":break;case"daccpcb":document.body.style.filter="saturate(150%)";break;case"daccpds":document.body.style.fontFamily="OpenDyslexic, Arial, sans-serif";document.body.style.lineHeight="1.5";break;case"daccplv":document.body.style.fontSize="120%";document.body.style.filter="saturate(150%)";break;case"daccpep":document.body.style.animation="none";document.body.style.filter="saturate(50%)";break;case"daccpad":document.body.style.animation="none";document.body.style.filter="saturate(50%)";break;case"daccpcal":document.body.style.fontSize="110%";document.body.style.animation="none";break}})});document.querySelectorAll("#daccbxc2 button").forEach(btn=>{btn.addEventListener("click",function(){const action=this.getAttribute("id");switch(action){case"daccfs":const currentSize=parseFloat(getComputedStyle(document.body).fontSize);document.body.style.fontSize=currentSize+2+"px";break;case"daccul":document.querySelectorAll("a").forEach(a=>a.style.textDecoration="underline");break;case"daccls":document.body.style.letterSpacing="1px";break;case"dacclh":document.body.style.lineHeight="1.5";break;case"dacctts":break}})});document.querySelectorAll("#daccbxc3 button").forEach(btn=>{btn.addEventListener("click",function(){const action=this.getAttribute("id");switch(action){case"daccic":document.body.style.filter="invert(100%)";break;case"dacccc":document.body.style.backgroundColor="#000";document.body.style.color="#ff0";break;case"daccsat":document.body.style.filter="saturate(150%)";break}})});document.querySelectorAll("#daccbxc4 button").forEach(btn=>{btn.addEventListener("click",function(){const action=this.getAttribute("id");switch(action){case"daccda":document.body.style.animation="none";document.body.style.transition="none";break}})})}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",function(){loadStyles();createAccessibilityWidget();initializeWidget()})}else{loadStyles();createAccessibilityWidget();initializeWidget()}})();